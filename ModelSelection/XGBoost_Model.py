...
# %%
# Randomized Search CV param_distributions for XGBoost
...
colsample_bytree = [round(x, 2) for x in np.linspace(0.50, 1.00, num = 5)]
# subsample: float -> 0.50-1.00
subsample = [round(x, 2) for x in np.linspace(0.50, 1.00, num = 5)]
...
# %%
if not os.path.exists('xgboost_regression_model.pkl'):
    # Run time: 29 min
...
randomized_search_cv = pickle.load(open('xgboost_regression_model.pkl','rb'))
